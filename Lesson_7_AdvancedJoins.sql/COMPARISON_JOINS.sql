...
--Write a query that left joins the accounts table and the sales_reps tables on each sale rep's ID and joins it using the < comparison operator on accounts.primary_poc and sales_reps.name. 
 SELECT a.name account_name, 
		...
		s.name sales_rep
 FROM accounts a
 LEFT JOIN sales_reps s  --note that even if you are left joining accounts tables, you have to select account table first.
 ON s.id = a.sales_rep_id
 	AND a.primary_poc < s.name --arranges alphabetically
